...
forge verify-contract <CONTRACT_ADDRESS> <PATH>:<CONTRACT_NAME> \
  --chain-id %%active.chainId%% \
  --verifier-url %%active.explorerApi%%/api \
  --verifier custom \
...
| `--verifier-url` | <NetworkValue field="explorerApi" path="/api" /> | BattleChain explorer API |
...
| `--rpc-url` | <NetworkValue field="rpc" /> | BattleChain RPC |

On success you'll see:

```
Start verifying contract `0x...` deployed on %%active.chainId%%

Submitting verification for [src/MyVault.sol:MyVault] 0x...
Submitted contract for verification:
        Response: `OK`
        GUID: `<guid>`
        URL: %%active.explorerApi%%/address/0x...
```
