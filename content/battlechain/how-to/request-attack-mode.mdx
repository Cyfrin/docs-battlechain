...
contract RequestAttackMode is Script {
    IAttackRegistry constant REGISTRY =
        IAttackRegistry(%%testnet.attackRegistry%%);
...
Call `requestUnderAttack` on the `AttackRegistry` (`%%mainnet.attackRegistry%%` on mainnet):

```bash
cast send %%mainnet.attackRegistry%% \
  "requestUnderAttack(address)" $AGREEMENT_ADDRESS \
  --account battlechain \
  --rpc-url %%mainnet.rpc%%
```
...
```bash
cast call %%mainnet.attackRegistry%% \
  "getAgreementState(address)(uint8)" $AGREEMENT_ADDRESS \
  --rpc-url %%mainnet.rpc%%
```
...
<Note>
Track your request without scripting on the **[Approvals dashboard](https://approvals.battlechain.com)** — your agreement appears there as soon as it's requested, showing its current state and full details for moderators to review.
</Note>
