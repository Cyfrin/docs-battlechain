...
<Tip>
Prefer a UI? The **[Approvals dashboard](https://approvals.battlechain.com)** lists every pending request with full protocol details, and lets you approve or reject directly once you connect the moderator Safe — no `cast` needed. The steps below show the equivalent on-chain calls.
</Tip>
...
```bash
cast call %%testnet.attackRegistry%% \
...
```bash
cast send %%testnet.attackRegistry%% \
