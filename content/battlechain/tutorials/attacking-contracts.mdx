...
If you're attacking someone else's vault, you need their `VAULT_ADDRESS` and `TOKEN_ADDRESS` (find these on the <NetworkValue field="explorer" href label="BattleChain explorer" /> or by querying the `AttackRegistry`).
...
Search your `VAULT_ADDRESS` on the <NetworkValue field="explorer" href label="BattleChain explorer" />. You should see:
